...
def get_completion_from_messages(messages, model="llama3.1", temperature=0, max_tokens=500):
...
def find_category_and_product_only(user_input, products_and_category):
    """
    Uses Ollama LLaMA 3 to extract categories and products.
...
def read_string_to_list(input_data):
    ...
    try:
        ...
        cleaned_input = input_data.strip().strip("```").replace("json", "").strip()
        return json.loads(cleaned_input)  # Convert string to list
